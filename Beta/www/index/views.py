...
def indexView(request):
    ...
    # 对banner模板提前进行渲染再传递显示到index.html
...
def scriptdisplayView(request, scriptId):
    ...
    if request.method == 'GET':
        ...
        scriptTemplate = loader.get_template(os.path.join(os.getcwd(), 'script', script[0].script_label.label_path, script[0].script_file.split('.')[0] + ".html"))
        # 把脚本的ID传入到渲染的模板中，让调用脚本时候能够识别是调用哪个脚本
        # 不传脚本名称的原因是因为不能类别的脚本工具可能会存在同名现象
        display = scriptTemplate.render({'script_id': script[0].script_id}, request)
        return render(request, "index.html", locals())
    else:
...
def scriptcallView(request):
    ...
    if request.method == "POST":
        # 从请求体从获取参数,并过滤掉b 和 '
        request_body = str(request.body).strip('b').replace("'", "")
        # 用来存放从请求体中获取的参数
        args_dic = {}
        # 临时参数列表,把请求体中的参数通过&分割开来
        tmp_list = request_body.split("&")
        for i in range(len(tmp_list)):
            # 以参数名为字典的键，参数的值为字典的值存入到字典
            args_dic[tmp_list[i].split('=')[0]] = tmp_list[i].split('=')[1]
        # 根据表单中的scriptID字段从数据库中读取要调用脚本的信息
        script = Script.objects.select_related('script_label').get(script_id=args_dic['scriptID'])
        # 开始组合命令
        # command用来存放运行的命令
        command = ""
        # 先识别脚本是用什么语言写的
        ...
        del args_dic['csrfmiddlewaretoken']
...
def tasksqueueView(request):
    ...
    if request.method == "POST":
        ...
        tmp_list = request_body.split("&")
        for i in range(len(tmp_list)):
            # 先判断参数的参数值是否未空，为空则不加入
            if tmp_list[i].split('=')[1] == '':
                pass
            else:
                # 以参数名为字典的键，参数的值为字典的值存入到字典
                args_dic[tmp_list[i].split('=')[0]] = tmp_list[i].split('=')[1]
        # 根据表单中的scriptID字段从数据库中读取要调用脚本的信息
        script = Script.objects.select_related(
            'script_label').get(script_id=args_dic['scriptID'])
        # 开始组合命令
        # command用来存放运行的命令
        command = ""
        # 先识别脚本是用什么语言写的
        ...
        # 任务开始的时间
        # 因为本地时间跟celery的时间会有点误差，通过datetime.timedelta来进行调整
